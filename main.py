...
def write_solution_on_image(image, grid, user_grid):
    ...
    for i in range(SIZE):
        for j in range(SIZE):
            ...
            font_scale = 0.6 * min(width, height) / \
                max(text_height, text_width)
            text_height *= font_scale
            text_width *= font_scale
            bottom_left_corner_x = width*j + \
                math.floor((width - text_width) / 2) + off_set_x
            bottom_left_corner_y = height * \
                (i+1) - math.floor((height - text_height) / 2) + off_set_y
...
if __name__ == "__main__":
    ...
    board, corners, img, warped_img = get_sudoku(
        path1, show_contours=False, show_corners=False, show_grid=False)
    print(board)
