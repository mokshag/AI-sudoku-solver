...
def read_process_image(path):
    ...
    # gaussian blurring
    # Tried other values but found (9,9)is the best kernel size.
    kernel_size = (9, 9)
    ...
    # erosion followed by dilation (cv2.MORPH_OPEN)
...
def get_cropimage(img, orig_img, corners):
    ...
    output_pts = np.array([[0, 0], [length, 0], [0, length], [
                          length, length]], dtype='float32')
...
def display_gridlines(img, color=(255, 0, 0)):
    ...
    for i in range(9):
        img = cv2.line(img, (floor(side*(i+1)), 0),
                       (floor(side*(i+1)), floor(side*9)), color)
        img = cv2.line(img, (0, floor(side*(i+1))),
                       (floor(side*9), floor(side*(i+1))), color)
    show_image(img)
